...
var MatchTable, likenessCompare, listLengthSummary, mkElt, numKeys, numMatches, solvePuzzle;
...
listLengthSummary = function(hash) {
  var likeness, words;
  return ((function() {
    var results;
    results = [];
    for (likeness in hash) {
      words = hash[likeness];
...
likenessCompare = function(hash1, hash2) {
  var first1, first2, summary1, summary2;
  summary1 = listLengthSummary(hash1);
  summary2 = listLengthSummary(hash2);
  while (summary1.length > 0 && summary2.length > 0) {
    first1 = summary1.pop();
    first2 = summary2.pop();
    if (first1 < first2) {
      return -1;
...
MatchTable = (function() {
  ...
  MatchTable.prototype.wordsToTry = function() {
    ...
    return this.words.sort(function(a, b) {
      return likenessCompare(matchTable[a], matchTable[b]);
    });
  };

  MatchTable.prototype.toElt = function() {
    var elt, headerRow, j, k, lastRow, len1, len2, likeness, likenessButton, likenessCell, other, otherCell, others, ref, resetButton, resetCell, row, word, wordList, wordRowCount;
    elt = mkElt('table', {
    ...
    for (j = 0, len1 = ref.length; j < len1; j++) {
      ...
      wordRowCount = 0;
      for (likeness in others) {
        wordList = others[likeness];
        row = mkElt('tr');
        if (wordRowCount === 0) {
        ...
        if (wordList.length > 1) {
          ...
          this.addButtonClick(likenessButton, wordList);
...
//# sourceMappingURL=fallout-hacker.js.map
